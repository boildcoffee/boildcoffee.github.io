...
```java
    ...
    //单选
    datePicker.setOnSingleSelectedListener(new DatePicker.OnSingleSelectedListener() {
            @Override
            public void onSingleSelected(MonthView yjMonthView, MonthView.DayCell dayCell) {
                Toast.makeText(MainActivity.this,dayCell.getYear() + "-" + dayCell.getMonth() + "-" + dayCell.getDay(),Toast.LENGTH_LONG).show();
            }
        });
    //多选
//        datePicker.setOnMultiSelectedListener(new DatePicker.OnMultiSelectedListener() {
//            @Override
//            public void onMultiSelected(MonthView yjMonthView, List<MonthView.DayCell> dayCells) {
//                Toast.makeText(MainActivity.this,dayCells.toString(),Toast.LENGTH_LONG).show();
//            }
//        });
```
...
1. 通过`drawHeaderSpace(canvas)`方法绘制顶灰色部间隔
2. 通过`drawHeaderYM(canvas)`绘制年月
3. 通过`drawCell(canvas)`绘制事先由`generateData()`方法计算并生成好的每个日期
4. 重载`onTouchEvent(MotionEvent event)`实现单选和多选
...
public class MonthView extends View{
                ...
                if (distance <= 15){
                    ...
                    if (collisionDayCell != null && collisionDayCell.day > 0){
                        if (mOnSelectedListener != null){
                            mOnSelectedListener.onSelected(MonthView.this,collisionDayCell);
                        }
                        postInvalidate();
                        performClick();
                    }
    ...
    public interface OnSelectedListener{
        void onSelected(MonthView monthView,MonthView.DayCell dayCell);
    }
}
```
...
```java
  public class DatePicker extends LinearLayout {
    ...
    private List<MonthView.DayCell> mSelectedList = new ArrayList<>();
    ...
    private void initView(Context context) {
        ...
        BaseQuickAdapter<String,BaseViewHolder> adapter = new BaseQuickAdapter<String, BaseViewHolder>(R.layout.item_date_pick,mYM) {
            ...
            protected void convert(BaseViewHolder helper, String item) {
                ...
                if (mOnSingleSelectedListener != null || mOnMultiSelectedListener != null){
                    monthView.setSelectedList(mSelectedList);
                    monthView.setOnSelectedListener(new MonthView.OnSelectedListener() {
                        @Override
                        public void onSelected(MonthView monthView, MonthView.DayCell dayCell) {
                            if (mOnSingleSelectedListener != null){
                                mSelectedList.clear();
                                mSelectedList.add(dayCell);
                                mOnSingleSelectedListener.onSingleSelected(monthView,dayCell);
                            }else {
                                mSelectedList.add(dayCell);
                                mOnMultiSelectedListener.onMultiSelected(monthView,mSelectedList);
                            }
    ...
    public interface OnSingleSelectedListener{
        void onSingleSelected(MonthView yjMonthView,MonthView.DayCell dayCell);
    }

    public interface OnMultiSelectedListener {
        void onMultiSelected(MonthView yjMonthView,List<MonthView.DayCell> dayCells);
    }
}
```
...
https://github.com/aii1991/DatePicker